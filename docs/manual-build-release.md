...
2. `versions.json` -> add `"X.Y.Z": "<minAppVersion>"`
3. `src/static/manifest.json` -> `"version": "X.Y.Z"`
4. `src/static/versions.json` -> add `"X.Y.Z": "<minAppVersion>"`
5. `package.json` -> `"version": "X.Y.Z"`

Rule:

- git tag must be `X.Y.Z` and must match `manifest.json.version`.
...
`release.yml` starts on pushed tags:

1. Checks tag version matches `manifest.json.version`.
2. Runs `npm ci`.
3. Runs `npm run build`.
4. Creates `graphfrontier-<version>.zip` from `dist/`.
5. Uploads release assets to GitHub Release (`graphfrontier-<version>.zip`, `main.js`, `manifest.json`, `styles.css`).
