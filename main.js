...
// Default persisted state loaded on startup.
const DEFAULT_DATA = {
  pins: {},
  orbit_pins: {},
  saved_positions: {},
...
// Command catalog used by hotkeys and command invocations.
...
// Hotkey alias normalization keeps key names consistent (for example esc -> Escape).
...
module.exports = class GraphFrontierPlugin extends Plugin {
  async onload() {
    ...
    this.addCommand({
      ...
      callback: () => this.openOrRevealGraphFrontierView(),
    });

    this.addCommand({
      id: 'graphfrontier-toggle-pin-under-cursor',
      name: 'Toggle pin under cursor (GraphFrontier)',
      callback: async () => {
        const view = this.getActiveGraphFrontierView();
        if (!view) {
          new Notice('Open GraphFrontier view');
          return;
        }
        await view.togglePinUnderCursor();
      },
    });

    this.addCommand({
      id: 'graphfrontier-set-force-multiplier-under-cursor',
      name: 'Set force multiplier under cursor (GraphFrontier)',
      callback: async () => {
        const view = this.getActiveGraphFrontierView();
        if (!view) {
          new Notice('Open GraphFrontier view');
        ...
        await view.promptSetMultiplierUnderCursor();
      },
    });

    this.addCommand({
      id: 'graphfrontier-clear-force-multiplier-under-cursor',
      name: 'Clear force multiplier under cursor (GraphFrontier)',
      callback: async () => {
        const view = this.getActiveGraphFrontierView();
        if (!view) {
          new Notice('Open GraphFrontier view');
        ...
        await view.clearMultiplierUnderCursor();
      },
    });

    this.addCommand({
      id: 'graphfrontier-align-pins-to-grid',
      name: 'Align pins to grid (GraphFrontier)',
      callback: async () => {
        const view = this.getActiveGraphFrontierView();
        if (!view) {
          new Notice('Open GraphFrontier view');
        ...
        await view.alignPinsToGrid();
      },
    });

    // Commands for pin and orbit actions on node under cursor.
    this.addCommand({
      id: 'graphfrontier-pin-node-under-cursor',
      name: 'Pin node under cursor (GraphFrontier)',
      callback: async () => {
        const view = this.getActiveGraphFrontierView();
        if (!view) return new Notice('Open GraphFrontier view');
        await view.commandPinNode();
      },
    });

    this.addCommand({
      id: 'graphfrontier-pin-to-grid-under-cursor',
      name: 'Pin node to grid under cursor (GraphFrontier)',
      callback: async () => {
        const view = this.getActiveGraphFrontierView();
        if (!view) return new Notice('Open GraphFrontier view');
        await view.commandPinToGrid();
      },
    });

    this.addCommand({
      id: 'graphfrontier-unpin-node-under-cursor',
      name: 'Unpin node under cursor (GraphFrontier)',
      callback: async () => {
        const view = this.getActiveGraphFrontierView();
        if (!view) return new Notice('Open GraphFrontier view');
        await view.commandUnpinNode();
      },
    });

    this.addCommand({
      id: 'graphfrontier-pin-linked-to-orbit-under-cursor',
      name: 'Pin linked to orbit under cursor (GraphFrontier)',
      callback: async () => {
        const view = this.getActiveGraphFrontierView();
        if (!view) return new Notice('Open GraphFrontier view');
        await view.commandPinLinkedToOrbit();
      },
    });

    this.addCommand({
      id: 'graphfrontier-unpin-linked-nodes-under-cursor',
      name: 'Unpin linked nodes under cursor (GraphFrontier)',
      callback: async () => {
        const view = this.getActiveGraphFrontierView();
        if (!view) return new Notice('Open GraphFrontier view');
        await view.commandUnpinLinkedNodes();
      },
    });

    this.addCommand({
      id: 'graphfrontier-add-to-search-under-cursor',
      name: 'Add to search under cursor (GraphFrontier)',
      callback: async () => {
        const view = this.getActiveGraphFrontierView();
        if (!view) return new Notice('Open GraphFrontier view');
        await view.commandAddToSearch();
      },
    });

    this.addCommand({
      id: 'graphfrontier-show-local-graph-under-cursor',
      name: 'Show local graph under cursor (GraphFrontier)',
      callback: async () => {
        const view = this.getActiveGraphFrontierView();
        if (!view) return new Notice('Open GraphFrontier view');
        await view.commandShowLocalGraph();
      },
    });

    this.addCommand({
      id: 'graphfrontier-pin-linked-nodes-under-cursor',
      name: 'Pin linked nodes under cursor (GraphFrontier)',
      callback: async () => {
        const view = this.getActiveGraphFrontierView();
        if (!view) return new Notice('Open GraphFrontier view');
        await view.commandPinLinkedNodes();
      },
    });

    this.addCommand({
      id: 'graphfrontier-pin-linked-nodes-to-grid-under-cursor',
      name: 'Pin linked nodes to grid under cursor (GraphFrontier)',
      callback: async () => {
        const view = this.getActiveGraphFrontierView();
        if (!view) return new Notice('Open GraphFrontier view');
        await view.commandPinLinkedNodesToGrid();
      },
    });

    // Commands for visual emphasis and link-force actions.
    this.addCommand({
      id: 'graphfrontier-paint-edges-under-cursor',
      name: 'Paint edges under cursor (GraphFrontier)',
      callback: async () => {
        const view = this.getActiveGraphFrontierView();
        if (!view) return new Notice('Open GraphFrontier view');
        await view.commandPaintEdges();
      },
    });

    this.addCommand({
      id: 'graphfrontier-clear-painted-edges-under-cursor',
      name: 'Clear painted edges under cursor (GraphFrontier)',
      callback: async () => {
        const view = this.getActiveGraphFrontierView();
        if (!view) return new Notice('Open GraphFrontier view');
        await view.commandClearPaintedEdges();
      },
    });

    this.addCommand({
      id: 'graphfrontier-strong-pull-under-cursor',
      name: 'Strong pull under cursor (GraphFrontier)',
      callback: async () => {
        const view = this.getActiveGraphFrontierView();
        if (!view) return new Notice('Open GraphFrontier view');
        await view.commandStrongPull();
      },
    });

    this.addCommand({
      id: 'graphfrontier-clear-strong-pull-under-cursor',
      name: 'Clear strong pull under cursor (GraphFrontier)',
      callback: async () => {
        const view = this.getActiveGraphFrontierView();
        if (!view) return new Notice('Open GraphFrontier view');
        await view.commandClearStrongPull();
      },
    });

    // Commands for layout save/load and bulk operations.
    this.addCommand({
      id: 'graphfrontier-save-layout',
      name: 'Save layout (GraphFrontier)',
      callback: async () => {
        const view = this.getActiveGraphFrontierView();
        if (!view) return new Notice('Open GraphFrontier view');
        await view.saveCurrentLayout();
      },
    });

    this.addCommand({
      id: 'graphfrontier-load-layout',
      name: 'Load layout (GraphFrontier)',
      callback: async () => {
        const view = this.getActiveGraphFrontierView();
        if (!view) return new Notice('Open GraphFrontier view');
        await view.loadSavedLayout();
      },
    });

    this.addCommand({
      id: 'graphfrontier-pin-all',
      name: 'Pin all nodes (GraphFrontier)',
      callback: async () => {
        const view = this.getActiveGraphFrontierView();
        if (!view) return new Notice('Open GraphFrontier view');
        await view.commandPinAllNodes();
      },
    });

    this.addCommand({
      id: 'graphfrontier-unpin-all',
      name: 'Unpin all nodes (GraphFrontier)',
      callback: async () => {
        const view = this.getActiveGraphFrontierView();
        if (!view) return new Notice('Open GraphFrontier view');
        await view.commandUnpinAllNodes();
      },
    });
    ...
    this.registerEvent(this.app.metadataCache.on('resolved', () => this.scheduleRefreshAllViews()));
    this.registerEvent(this.app.vault.on('create', () => this.scheduleRefreshAllViews()));
    this.registerEvent(this.app.vault.on('modify', () => this.scheduleRefreshAllViews()));
    this.registerEvent(this.app.vault.on('delete', () => this.scheduleRefreshAllViews()));
    this.registerEvent(this.app.vault.on('rename', () => this.scheduleRefreshAllViews()));
    this.registerEvent(this.app.workspace.on('layout-change', () => this.scheduleRefreshAllViews()));
  }
  ...
  normalizeHotkeyText(rawHotkey) {
    ...
    for (const part of parts) {
      ...
      if (['mod', 'cmd', 'command', 'ctrl', 'control', 'meta'].includes(loweredPart)) {
        hasMod = true;
        continue;
      }
      if (['alt', 'option'].includes(loweredPart)) {
        hasAlt = true;
        continue;
      }
      if (loweredPart === 'shift') {
        hasShift = true;
        continue;
      }
      if (hotkeyKey) return '';
      hotkeyKey = this.normalizeHotkeyKey(part);
    }
  ...
  getHotkeyFromKeyboardEvent(keyboardEvent) {
    ...
    const hotkeyKey = this.normalizeHotkeyKey(keyboardEvent.key);
    if (!hotkeyKey) return '';
    const hotkeyParts = [];
    ...
    return this.normalizeHotkeyText(hotkeyParts.join('+'));
  }
 ...
 * ============================================================
 * VIEW BLOCK
 * UI layer: side panel, search rows, groups, menus, and interaction handlers.
 * ============================================================
 */
class GraphFrontierView extends ItemView {
  ...
  stepFocusSmoothing() {
    const targetFocusNodeId = this.getActiveFocusNodeId();
    const targetProgress = targetFocusNodeId ? 1 : 0;
    const smooth = 0.2;
    this.focusProgress += (targetProgress - this.focusProgress) * smooth;
    if (Math.abs(targetProgress - this.focusProgress) < 0.01) {
      this.focusProgress = targetProgress;
    }
    if (targetFocusNodeId) this.focusNodeId = targetFocusNodeId;
    else if (this.focusProgress <= 0.001) this.focusNodeId = null;

    const baseSearchFocusNodeId = this.getFilterNodeId() || this.getFindFocusNodeId();
    const hoverTargetNodeId = (
      baseSearchFocusNodeId
      && this.hoverNodeId
      && this.hoverNodeId !== baseSearchFocusNodeId
    ) ? this.hoverNodeId : null;

    if (hoverTargetNodeId && hoverTargetNodeId !== this.hoverFocusNodeId) {
      if (this.hoverFocusNodeId && this.hoverFocusProgress > 0.001) {
        this.hoverFadeNodeId = this.hoverFocusNodeId;
        this.hoverFadeProgress = Math.max(this.hoverFadeProgress, this.hoverFocusProgress);
      }
      this.hoverFocusNodeId = hoverTargetNodeId;
      this.hoverFocusProgress = 0;
      if (this.hoverFadeNodeId && this.hoverFadeNodeId === this.hoverFocusNodeId) {
        this.hoverFadeNodeId = null;
        this.hoverFadeProgress = 0;
      }
    }
    ...
    const hoverSmooth = targetHoverProgress > this.hoverFocusProgress ? 0.24 : 0.3;
    this.hoverFocusProgress += (targetHoverProgress - this.hoverFocusProgress) * hoverSmooth;
    if (Math.abs(targetHoverProgress - this.hoverFocusProgress) < 0.01) {
      this.hoverFocusProgress = targetHoverProgress;
    }
    if (!hoverTargetNodeId && this.hoverFocusProgress <= 0.001) {
      this.hoverFocusNodeId = null;
    }

    const targetHoverFadeProgress = 0;
    const hoverFadeSmooth = 0.3;
    this.hoverFadeProgress += (targetHoverFadeProgress - this.hoverFadeProgress) * hoverFadeSmooth;
    if (Math.abs(targetHoverFadeProgress - this.hoverFadeProgress) < 0.01) {
      this.hoverFadeProgress = targetHoverFadeProgress;
    }
    if (this.hoverFadeProgress <= 0.001) {
      this.hoverFadeNodeId = null;
    }
  }
  ...
  refreshFromVault(opts = {}) {
    ...
    for (const rawNode of graphData.nodes) {
      const old = oldNodes.get(rawNode.id);
    ...
    for (const rawEdge of graphData.edges) {
  ...
  updateLayoutCenter() {
    if (this.nodes.length === 0) {
      this.layoutCenter.x = 0;
      this.layoutCenter.y = 0;
      return;
    }
    ...
    if (mainCount > 0) {
      this.layoutCenter.x = sumMainX / mainCount;
      this.layoutCenter.y = sumMainY / mainCount;
      return;
    }
    ...
    this.layoutCenter.x = sumAllX / this.nodes.length;
    this.layoutCenter.y = sumAllY / this.nodes.length;
  }
 ...
 */
  ...
  stepCameraSmoothing() {
    const smooth = 0.22;
    this.camera.x += (this.cameraTarget.x - this.camera.x) * smooth;
    this.camera.y += (this.cameraTarget.y - this.camera.y) * smooth;
    this.camera.zoom += (this.cameraTarget.zoom - this.camera.zoom) * smooth;
  }
  ...
  stepSimulation() {
    ...
    const elapsedSearchMs = nowMs - this.layoutKickAtMs;
    const baseSearchFactor = Math.max(0, Math.min(1, 1 - (elapsedSearchMs / layoutSearchMs)));
    const layoutSearchFactor = this.dragNodeId ? 1 : baseSearchFactor;
    ...
    const linkDistance = this.plugin.clampNumber(settings.link_distance, 1, 100, DEFAULT_DATA.settings.link_distance);
    const attachmentLinkDistance = this.plugin.clampNumber(
      settings.attachment_link_distance_multiplier,
      1,
      100,
      DEFAULT_DATA.settings.attachment_link_distance_multiplier,
    );
    const damping = this.plugin.clampNumber(settings.damping, 0.01, 0.9, DEFAULT_DATA.settings.damping);
    const hasRepel = repelStrength > 0;
    const hasCenter = centerStrength > 0;
    const hasLink = baseLinkStrength > 0 && this.edges.length > 0;
    ...
    for (const node of nodes) {
      const orbitPin = this.plugin.getOrbitPin(node.id);
      if (!orbitPin) continue;
      const anchorNode = this.nodeById.get(orbitPin.anchor_id);
      if (!anchorNode) continue;
      orbitPinsById.set(node.id, orbitPin);
    }
    const orbitNodeIds = new Set(orbitPinsById.keys());
    const autoAttachmentOrbitById = this.buildAttachmentAutoOrbitMap();
    ...
    if (hasLink) {
      for (const edge of this.edges) {
        ...
        const sourceMultiplier = this.plugin.getNodeMultiplier(sourceNode.id);
        const targetMultiplier = this.plugin.getNodeMultiplier(targetNode.id);
    ...
    for (const node of nodes) {
      const pin = this.plugin.getPin(node.id);
      ...
      if (hasCenter && !pin && !orbitPin && !autoAttachmentOrbit && node.id !== this.dragNodeId) {
        const nodeCenterScale = node.degree === 0 ? orphanCenterScale : 1;
        const centerFx = (this.layoutCenter.x - node.x) * centerStrength * nodeCenterScale;
        const centerFy = (this.layoutCenter.y - node.y) * centerStrength * nodeCenterScale;
      ...
      if (autoAttachmentOrbit) {
        const anchorNode = this.nodeById.get(autoAttachmentOrbit.anchor_id);
        if (anchorNode) {
    ...
    const autosaveAllowed = settings.layout_autosave && !this.isSearchFilled();
    if (autosaveAllowed) {
      if (movingNodeCount > 0) {
        this.layoutStillFrames = 0;
        this.layoutAutosaveDirty = true;
      } else {
        this.layoutStillFrames += 1;
      }

      if (this.layoutAutosaveDirty && this.layoutStillFrames >= 8) {
        this.saveCurrentLayout({ silent: true });
        this.layoutAutosaveDirty = false;
      }
    } else {
      this.layoutStillFrames = 0;
      if (this.isSearchFilled()) this.layoutAutosaveDirty = false;
    }
  }
 ...
 */
  render() {
    ...
    if (this.plugin.getSettings().show_grid) {
      this.drawGrid(ctx);
    }
  ...
  drawGrid(ctx) {
    ...
    const rightWorld = this.screenToWorld(this.viewWidth, 0).x;
    const topWorld = this.screenToWorld(0, 0).y;
    const bottomWorld = this.screenToWorld(0, this.viewHeight).y;
  ...
  getHoverDimAlpha(focusProgress) {
    ...
    const dimStrength = this.plugin.clampNumber(
      this.plugin.getSettings().hover_dim_strength,
      0,
      100,
      DEFAULT_DATA.settings.hover_dim_strength,
    );
  ...
  drawEdges(ctx) {
    const edgeScale = this.plugin.clampNumber(
      this.plugin.getSettings().edge_width_scale,
      0.01,
      1,
      DEFAULT_DATA.settings.edge_width_scale,
    );
    const paintedEdgeScale = this.plugin.clampNumber(
    ...
    const focusProgress = this.focusProgress;
    const hasFocus = !!focusNodeId && focusProgress > 0.001;
    const hoverFocusNodeId = this.hoverFocusNodeId;
    const hoverFocusProgress = this.hoverFocusProgress;
    const hasHoverFocus = !!hoverFocusNodeId && hoverFocusProgress > 0.001;
    const hoverFadeNodeId = this.hoverFadeNodeId;
    const hoverFadeProgress = this.hoverFadeProgress;
    ...
    for (const edge of this.edges) {
      ...
      const sourcePoint = this.worldToScreen(sourceNode.x, sourceNode.y);
      const targetPoint = this.worldToScreen(targetNode.x, targetNode.y);
      const sourcePaintColor = this.plugin.getPaintedEdgeColor(sourceNode.id);
      const targetPaintColor = this.plugin.getPaintedEdgeColor(targetNode.id);
      ...
      const dimAlpha = this.getHoverDimAlpha(combinedFocusProgress);
      ...
      if (paintedColor) {
        ...
        ctx.strokeStyle = this.hexToRgba(paintedColor, alpha);
  ...
  drawNodes(ctx) {
    const zoom = this.camera.zoom;
    const labelMinZoom = this.getLabelZoomThreshold();
    ...
    const focusProgress = this.focusProgress;
    const hasFocus = !!focusNodeId && focusProgress > 0.001;
    const focusNeighbors = hasFocus ? (this.neighborsById.get(focusNodeId) || new Set()) : new Set();
    const hoverFocusNodeId = this.hoverFocusNodeId;
    const hoverFocusProgress = this.hoverFocusProgress;
    const hasHoverFocus = !!hoverFocusNodeId && hoverFocusProgress > 0.001;
    const hoverFadeNodeId = this.hoverFadeNodeId;
    const hoverFadeProgress = this.hoverFadeProgress;
    const hasHoverFade = !!hoverFadeNodeId && hoverFadeProgress > 0.001;
    const hoverFocusNeighbors = hasHoverFocus ? (this.neighborsById.get(hoverFocusNodeId) || new Set()) : new Set();
    const hoverFadeNeighbors = hasHoverFade ? (this.neighborsById.get(hoverFadeNodeId) || new Set()) : new Set();
    const hasAnyFocus = hasFocus || hasHoverFocus || hasHoverFade;
    const combinedFocusProgress = Math.max(focusProgress, hoverFocusProgress, hoverFadeProgress);
    const hasSearchFocus = !!(this.getFilterNodeId() || this.getFindFocusNodeId());

    for (const node of this.nodes) {
      if (hasFilter && (!visibleNodeIds || !visibleNodeIds.has(node.id))) continue;
      const point = this.worldToScreen(node.x, node.y);
      if (point.x < -80 || point.x > this.viewWidth + 80 || point.y < -80 || point.y > this.viewHeight + 80) {
        continue;
      }

      const radius = Math.max(0.6, this.getNodeRadius(node) * this.camera.zoom);

      const isFocusNode = hasFocus && node.id === focusNodeId;
      const isHoverFocusNode = hasHoverFocus && node.id === hoverFocusNodeId;
      const isHoverNodeRaw = this.hoverNodeId === node.id;
      ...
      const isClickFlash = this.clickFlashNodeId === node.id && nowMs < this.clickFlashUntilMs;
    ...
    const hoverTitleNode = this.hoverNodeId ? (this.nodeById.get(this.hoverNodeId) || null) : null;
    if (hoverTitleNode) this.drawFocusedNodeTitle(ctx, hoverTitleNode);
    if (this.clickFlashNodeId && nowMs >= this.clickFlashUntilMs) {
      this.clickFlashNodeId = null;
      this.clickFlashUntilMs = 0;
    }
  }
  ...
  drawFocusedNodeTitle(ctx, node) {
    ...
    const boxX = Math.max(6, Math.min(this.viewWidth - boxWidth - 6, point.x - boxWidth / 2));
  ...
  getOrbitRadiusBySpacing(orbitNodeCount, minRadius) {
    const orbitDistance = this.plugin.clampNumber(
      this.plugin.getSettings().orbit_distance,
      1,
      100,
      DEFAULT_DATA.settings.orbit_distance,
    );
    const safeMinRadius = this.plugin.clampNumber(minRadius, 1, 100, 1);
  ...
  getOrbitRadiusForAnchor(orbitNodeCount) {
    const linkDistance = this.plugin.clampNumber(
      this.plugin.getSettings().link_distance,
      1,
      100,
      DEFAULT_DATA.settings.link_distance,
    );
    return this.getOrbitRadiusBySpacing(orbitNodeCount, linkDistance);
  }
  ...
  buildAttachmentAutoOrbitMap() {
    ...
    for (const node of this.nodes) {
      ...
      if (this.plugin.isPinned(node.id) || this.plugin.isOrbitPinned(node.id)) continue;

      const neighborIds = this.neighborsById.get(node.id);
      if (!neighborIds || neighborIds.size === 0) continue;

      let anchorId = null;
      for (const candidateId of neighborIds) {
        const candidateNode = this.nodeById.get(candidateId);
      ...
      if (!anchorId) {
        for (const candidateId of neighborIds) {
          if (this.nodeById.has(candidateId)) {
            anchorId = candidateId;
            break;
          }
        }
      }
      if (!anchorId || !this.nodeById.has(anchorId)) continue;
    ...
    for (const [anchorId, attachmentNodeIds] of attachmentNodeIdsByAnchor.entries()) {
      ...
      const attachmentOrbitRadius = this.getOrbitRadiusBySpacing(attachmentCount, attachmentLinkDistance);
  ...
  recomputeOrbitRadii() {
    const orbitPins = this.plugin.data.orbit_pins || {};
    const orbitNodeIdsByAnchor = new Map();

    for (const [orbitNodeId, orbitMeta] of Object.entries(orbitPins)) {
      const anchorId = String(orbitMeta?.anchor_id || '').trim();
      if (!anchorId) continue;
      if (!this.nodeById.has(orbitNodeId)) continue;
      if (!this.nodeById.has(anchorId)) continue;
    ...
    for (const [anchorId, orbitNodeIds] of orbitNodeIdsByAnchor.entries()) {
      const orbitRadius = this.getOrbitRadiusForAnchor(orbitNodeIds.length);
      const anchorNode = this.nodeById.get(anchorId);
      if (!anchorNode) continue;
      ...
      for (const orbitNodeId of orbitNodeIds) {
        ...
        const orbitNode = this.nodeById.get(orbitNodeId);
  ...
  getNodeRadius(node) {
    const nodeScaleRaw = this.plugin.clampNumber(
      this.plugin.getSettings().node_size_scale,
      0.1,
      2,
      DEFAULT_DATA.settings.node_size_scale,
    );
    const attachmentSizeMultiplier = this.plugin.clampNumber(
      this.plugin.getSettings().attachment_size_multiplier,
      0.1,
      1,
      DEFAULT_DATA.settings.attachment_size_multiplier,
    );
  ...
  getLabelZoomThreshold() {
    const steps = this.plugin.clampNumber(
      this.plugin.getSettings().label_zoom_steps,
      1,
      20,
      DEFAULT_DATA.settings.label_zoom_steps,
    );
  ...
  getLabelFontSize() {
    const baseSize = this.plugin.clampNumber(
      this.plugin.getSettings().label_font_size,
      5,
      20,
  ...
  getGroupColorForNode(node) {
    const groups = Array.isArray(this.plugin.data.groups) ? this.plugin.data.groups : [];
    ...
    for (const group of groups) {
      ...
      const parsed = this.plugin.parseGroupQuery(group.query);
      if (!parsed) continue;
      if (this.nodeMatchesParsedGroup(meta, parsed)) {
        return this.plugin.normalizeGroupColor(group.color);
      }
    }
    return null;
  }
